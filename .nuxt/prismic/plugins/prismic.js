...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = Prismic.client('https://bioceres-semillas.prismic.io/api/v2', Object.assign({}, options,  {}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://bioceres-semillas.prismic.io/api/v2'
      },
      predicate() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady((app) => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=bioceres-semillas&new=true'
